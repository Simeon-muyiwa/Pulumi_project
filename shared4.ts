...
const validateThumbprint = (secret: pulumi.Output<string>, expected: string) => {
  return secret.apply(t => {
    if (t && t !== expected) throw new Error(`Invalid thumbprint, expected ${expected}`);
    return t;
  });
};
...
const clusterCaThumbprint = config.requireSecret("clusterCaThumbprint");
...
const clusterOidc = clusterCaThumbprint.apply(thumbprint => {
  if (!thumbprint) {
    return pulumi.output({ arn: pulumi.output("") });
  }
  return new aws.iam.OpenIdConnectProvider("cluster", {
...
const createIRSARole = (
  ...
  const role = new aws.iam.Role(`irsa-${serviceName}`, {
    ...
    assumeRolePolicy: pulumi.all([clusterOidc, baseConfig.domain]).apply(([oidc, domain]) => {
      if (!oidc.arn) {
        return JSON.stringify({
          Version: "2012-10-17",
          Statement: [] // Empty policy until OIDC is ready
        });
      }
      return JSON.stringify({
        Version: "2012-10-17",
...
// =====================
export const irsaRoles = pulumi.all([clusterOidc]).apply(([oidc]) => {
  if (!oidc.arn) return {};
...
// Core Exports
// =================
export const coreExports = pulumi.all([clusterOidc, irsaRoles]).apply(([oidc, roles]) => ({
  clusterOidcUrl: pulumi.interpolate`https://oidc.${baseConfig.domain}`,
  githubOidcArn: githubOidc.arn,
  irsaRoleARNs: {
    ebsCSI: roles.ebsCSI?.arn || pulumi.output(""),
    clusterAutoscaler: roles.clusterAutoscaler?.arn || pulumi.output(""),
    cloudwatchAgent: roles.cloudwatchAgent?.arn || pulumi.output("")
  },
...
pulumi.all([coreExports.oidcReady]).apply(([ready]) => {
  if (config.getBoolean("requireOidc") && !ready) {
    throw new Error("OIDC configuration required but not available");
  }
});